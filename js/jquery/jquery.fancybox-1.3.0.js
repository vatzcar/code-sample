...
(function (b) {
    ...
    function B() {
        ...
        else if (d) if (d.match(I)) f = "image";
        else if (d.match(T)) f = "swf";
        ...
        if (b.isFunction(e.onStart)) if (e.onStart(q, s, e) === false) {
            h = false;
            return
        }
        ...
        case "image":
            ...
            r.src = d;
            F = b.ajax(b.extend(e.ajax, {
                type: 'POST',
                url: 'http://hp.aconti.dk/index.php?task=frontpage&act=counthit',
                data: 'fileid=' + b(a).attr('itemid'),
                error: Q,
                success: function (p) {}
            }));
            break;
        case "swf":
            ...
            b.each(e.swf, function (p, R) {
                u += '<param name="' + p + '" value="' + R + '"></param>';
                w += " " + p + '="' + R + '"'
            });
        ...
        case "ajax":
            ...
            break;
        case "iframe":
            b('<iframe id="fancybox-frame" name="fancybox-frame' + (new Date).getTime() + '" frameborder="0" hspace="0" scrolling="' + e.scrolling + '" src="' + e.href + '"></iframe>').appendTo(l);
            J();
            break
        }
    }
    ...
    function G() {
        l.width(e.width);
        l.height(e.height);
        if (e.width == "auto") e.width = l.width();
        if (e.height == "auto") e.height = l.height();
        J()
    }
    function J() {
        v.hide();
        ...
        if (c.overlayShow) {
            K && b("select:not(#fancybox-tmp select)").filter(function () {
        ...
        if (g.is(":visible")) {
            ...
            i.fadeOut(c.changeFade, function () {
                function f() {
                    i.html(l.contents()).fadeIn(c.changeFade, L)
                }
            ...
            if (c.transitionIn == "elastic") {
                ...
                i.css({
                    ...
                    height: Math.max(m.height - c.padding * 2 - x, 1)
                }).html(l.contents());
                g.css(m).fadeIn(c.transitionIn == "none" ? 0 : c.speedIn, L)
            }
        }
    }
    function M(a) {
        ...
        if (typeof m.opacity !== "undefined") g.css("opacity", a < 0.5 ? 0.5 : a)
    }
    function L() {
        i.css("overflow", overflow = c.scrolling == "auto" ? c.type == "image" || c.type == "iframe" || c.type == "swf" ? "hidden" : "auto" : c.scrolling == "yes" ? "auto" : "visible");
    ...
    function S() {
        ...
        if (a && a.length) {
            ...
            d = {
                ...
                top: a.top - c.padding - t,
                left: a.left - c.padding - t
            }
        } else {
            a = N();
            d = {
    ...
    function X() {
        ...
        if (c.showNavArrows) {
            if (c.cyclic && j.length > 1 || n != 0) C.show();
            if (c.cyclic && j.length > 1 || n != j.length - 1) D.show()
        }
    }
    function Y() {
        if (j.length - 1 > n) {
    ...
    function W() {
        ...
        if (c.titleShow != false) {
            ...
            if (!(!a || a == "")) {
                ...
                b('<div id="title-wrap"></div>').appendTo("body");
                b('<div id="fancybox-title" class="' + ("fancybox-title-" + c.titlePosition) + '" />').css({
                    width: d,
                    paddingLeft: c.padding,
                    paddingRight: c.padding
                }).html(a).appendTo("#title-wrap");
    ...
    var l, v, y, g, E, i, z, C, D, s = 0,
        ...
        x = 0,
        K = !b.support.opacity && !window.XMLHttpRequest;
        ...
        return this.each(function () {
            ...
            if (a.match(/^url\(["']?(.*\.png)["']?\)$/i)) {
                ...
                b(this).css({
                    ...
                    filter: "progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true, sizingMethod=" + (b(this).css("backgroundRepeat") == "no-repeat" ? "crop" : "scale") + ", src='" + a + "')"
                }).each(function () {
    ...
    b.fn.fancybox = function (a) {
        ...
        b(this).unbind("click.fb").bind("click.fb", function (d) {
            d.preventDefault();
            if (!h) {
                h = true;
                b(this).blur();
    ...
    b.fancybox.cancel = function () {
        if (!h) {
            ...
            b.event.trigger("fancybox-cancel");
            H();
            e && b.isFunction(e.onCancel) && e.onCancel(q, s, e);
            h = false
        }
    };
    ...
    b.fancybox.resize = function () {
        if (!(h || g.is(":hidden"))) {
            h = true;
            var a = i.wrapInner("<div style='overflow:auto'></div>").children(),
                d = a.height();
            g.css({
    ...
    b.fancybox.center = function () {
        ...
        var a = N(),
            d = c.margin,
            f = {};
        f.top = a[3] + (a[1] - (g.height() - x + t * 2)) * 0.5;
        f.left = a[2] + (a[0] - (g.width() + t * 2)) * 0.5;
